...
// PinsSetup0To27 sets the output current drive strength, output slew and input
// hysteresis for GPIO 0 to 27.
//
// Default drive is 8mA and both slew and hysteresis enabled. It is recommended
// to keep both slew and hysteresis enabled.
//
// Can only be used if driver bcm283x-dma was loaded.
func PinsSetup0To27(drive physic.Ampere, slew, hysteresis bool) error {
	if drvDMA.gpioPadMemory == nil {
		return errors.New("bcm283x-dma not initialized; try again as root?")
	}
	drvDMA.gpioPadMemory.pads0.set(toPad(drive, slew, hysteresis))
	return nil
}

// PinsSetup28To45 sets the output current drive strength, output slew and
// input hysteresis for GPIO 28 to 45.
//
// Default drive is 16mA and both slew and hysteresis enabled. It is recommended
// to keep both slew and hysteresis enabled.
//
// Can only be used if driver bcm283x-dma was loaded.
func PinsSetup28To45(drive physic.Ampere, slew, hysteresis bool) error {
	if drvDMA.gpioPadMemory == nil {
		return errors.New("bcm283x-dma not initialized; try again as root?")
	}
	drvDMA.gpioPadMemory.pads1.set(toPad(drive, slew, hysteresis))
	return nil
}
...
func (p *Pin) SlewLimit() bool {
	...
	if p.number < 28 {
		return drvDMA.gpioPadMemory.pads0&padSlewLimit != 0
	}
	return drvDMA.gpioPadMemory.pads1&padSlewLimit != 0
}
...
const (
	...
	padSlewLimit        pad = 1 << 4     // Output bandwidth limit to reduce bounce.
...
func toPad(drive physic.Ampere, slew, hysteresis bool) pad {
	var p pad
	switch {
	case drive <= 2:
		p = padDrive2mA
